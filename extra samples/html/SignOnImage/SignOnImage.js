...
// SignOnImage.js
//
// Demonstrates how to input a signature over the top of an existing image
//
...
function SignatureForm(preview) {
  ...
  this.connect = function () {
    ...
    //print("Devices found: " + usbDevices.Count);
    if (usbDevices.Count > 0) {
      ...
      if (usbDevice.idProduct != enumProductId.STU_300) {
		...
		var btnWidth = m_capability.screenWidth / m_btns.length;
      ...
      renderImage(usbDevice, useColor, true);
  ...
  function renderImage(usbDevice, useColor, includeImage)
  {
   // This application uses the same bitmap for both the screen and client (window).
      ...
      {	  
	     ...
	     var filename = chooseBackgroundImage(usbDevice);
	     //print("Displaying filename: " + filename);
	     document.getElementById("signatureImage").src = filename;  
	     var image = document.getElementById("signatureImage");
	     ctx.drawImage(image, 1, 1);
	  }
